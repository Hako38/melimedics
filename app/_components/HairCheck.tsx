...
export function HairCheck() {
  ...
  return <section className="hair-check-shell" aria-labelledby="hair-check-heading">
    ...
    <div className="hair-check-panel">
      ...
      {step === "contact" ? <div className="hair-contact-fields">
        <div className="hair-contact-grid">
          <label className="hair-text-field"><span>Vorname</span><input autoComplete="given-name" value={answers.firstName} onChange={(event) => update("firstName", event.target.value)} aria-describedby={errors.firstName ? "firstName-error" : undefined}/>{errors.firstName ? <b className="field-error" id="firstName-error" role="alert">{errors.firstName}</b> : null}</label>
          <label className="hair-text-field"><span>Nachname</span><input autoComplete="family-name" value={answers.lastName} onChange={(event) => update("lastName", event.target.value)} aria-describedby={errors.lastName ? "lastName-error" : undefined}/>{errors.lastName ? <b className="field-error" id="lastName-error" role="alert">{errors.lastName}</b> : null}</label>
          <label className="hair-text-field"><span>E-Mail</span><input type="email" autoComplete="email" value={answers.email} onChange={(event) => update("email", event.target.value)} aria-describedby={errors.email ? "email-error" : undefined}/>{errors.email ? <b className="field-error" id="email-error" role="alert">{errors.email}</b> : null}</label>
          <label className="hair-text-field"><span>Telefon</span><input type="tel" autoComplete="tel" value={answers.phone} onChange={(event) => update("phone", event.target.value)} aria-describedby={errors.phone ? "phone-error" : undefined}/>{errors.phone ? <b className="field-error" id="phone-error" role="alert">{errors.phone}</b> : null}</label>
        </div>
        <RadioGroup legend="Bevorzugte Kontaktmöglichkeit" optional name="preferredContact" options={preferredContactOptions} value={answers.preferredContact} onChange={(value) => update("preferredContact", value)}/>
        <label className="hair-text-field"><span>Zusätzliche Information <small>Optional</small></span><textarea maxLength={HAIR_CHECK_MAX_MESSAGE_LENGTH} value={answers.message} onChange={(event) => update("message", event.target.value)}/><small>{answers.message.length}/{HAIR_CHECK_MAX_MESSAGE_LENGTH}</small>{errors.message ? <b className="field-error" role="alert">{errors.message}</b> : null}</label>
        <div className="hair-consents">
          <p><strong>Ihre Einwilligung</strong> Sie entscheiden selbst, welche Angaben und optionalen Fotos Sie übermitteln.</p>
          <label><input type="checkbox" checked={answers.consent} onChange={(event) => update("consent", event.target.checked)}/><span>Ich willige ein, dass Melimedics meine Angaben zur Bearbeitung der Beratungsanfrage verarbeitet und mich über die angegebenen Kontaktdaten kontaktiert. Die Einwilligung kann ich jederzeit für die Zukunft widerrufen. <Link href="/datenschutz/">Datenschutzhinweise</Link></span></label>
          {errors.consent ? <p className="field-error" role="alert">{errors.consent}</p> : null}
          {photoCount > 0 ? <><label><input type="checkbox" checked={answers.photoConsent} onChange={(event) => update("photoConsent", event.target.checked)}/><span>Ich willige ausdrücklich ein, dass Melimedics meine ausgewählten Fotos als Gesundheitsdaten zur persönlichen Prüfung meiner Beratungsanfrage verarbeitet. Eine automatische Diagnose oder KI-Auswertung findet nicht statt.</span></label>{errors.photoConsent ? <p className="field-error" role="alert">{errors.photoConsent}</p> : null}</> : null}
        </div>
      </div> : null}
