...
const stepCopy: Record<Exclude<HairCheckStep, "start" | "review" | "success">, { eyebrow: string; title: string; intro: string }> = {
  ...
  photos: { eyebrow: "07 · Fotos", title: "Möchten Sie Aufnahmen vorbereiten?", intro: "Freiwillig. Die Bilder werden nur lokal in dieser Browser-Sitzung angezeigt und aktuell weder übertragen noch gespeichert." },
...
export function HairCheck() {
  ...
  if (step === "success") return <section className="hair-check-success" aria-labelledby="hair-check-success-title"><p className="eyebrow">Anfrage übermittelt</p><h1 id="hair-check-success-title" ref={headingRef} tabIndex={-1}>Vielen Dank – Ihre Angaben sind vorbereitet.</h1><p>Ihre Informationen können nun als Grundlage für die persönliche Beratung dienen.</p><div><Link className="button button-dark" href="/termin/" onClick={() => emitHairCheckEvent({ name: "hair_booking_clicked" })}>Termin vereinbaren</Link><Link className="button button-secondary" href="/haare/">Zur Haarmedizin</Link></div></section>;
