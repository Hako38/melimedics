...
export function CategoryPage({ eyebrow, title, intro, groups, path, note, showFinder = false, children }: { eyebrow: string; title: ReactNode; intro: string; groups: Group[]; path: string; note?: string; showFinder?: boolean; children?: ReactNode }) {
  return <PageShell>
    <Breadcrumbs items={[{ label: "Behandlungen", href: "/behandlungen/" }, { label: typeof title === "string" ? title : eyebrow, href: path }]}/>
    <InteriorHero eyebrow={eyebrow} title={title} intro={intro}>{showFinder ? <Link className="button button-dark" href="/behandlungsfinder/">Behandlung finden <span aria-hidden="true">↗</span></Link> : null}</InteriorHero>
